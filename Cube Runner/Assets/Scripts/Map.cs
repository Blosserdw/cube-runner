...
//					height = 0.6f;
	...
	{
		List<GameObject> pickupsList = new List<GameObject>(); // Hold pickups here so we can check if they're already being placed
		pickupsList.Clear();
		...
		for (int i = 0; i < GameManager.Instance.pickupsPerSide; i++)
		{
			...
			if (!pickupsList.Contains(Tiles[randomX, randomZ]))
			{
				// Set up random height
				float height = 0.0f;
				...
				pickupsList.Add(Tiles[randomX, randomZ]);
				transformableTiles.Remove(Tiles[randomX, randomZ]);
			}
			else
			{
				i--; // go through again to find a spot that isn't inhabitated.
			}
		}
	}
